 ...
 * is written to localStorage — a stale answer beats a spinner that never stops.
 */
...
/** Reads survive private mode, a wiped store and half-written JSON. */
...
function pruneOldest(): void {
  try {
    ...
    for (let i = 0; i < localStorage.length; i += 1) {
      ...
      entries.push({ key, at: readEntry(key.slice(CACHE_PREFIX.length))?.at ?? 0 })
    }
    ...
    // The store is gone entirely; callers already treat that as a miss.
  }
}
...
function writeEntry<T>(key: string, value: T): void {
  ...
  try {
    ...
    try {
      ...
      // Give up quietly: a cache miss next time is a slow app, not a broken one.
    }
  }
}

/**
 * Run `load` at most once per TTL, remembering the result in localStorage.
 *
