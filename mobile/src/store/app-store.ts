...
export const useApp = create<AppState & AppActions>((set, get) => ({
  ...
  allowLocate: () => {
    ...
    // A denied permission just keeps the default centre, which the farmer
    // can pan away from.
    set({ located: true })
    ...
    ;(async () => {
      try {
        ...
        set({ locatedAt: [pos.coords.latitude, pos.coords.longitude] })
      } catch {
        // No fix (emulator, airplane mode) — the default centre stands.
      }
    })()
  },
  setPts: (pts) => set({ pts }),
